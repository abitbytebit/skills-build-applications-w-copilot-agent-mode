...
router.get('/teams', async (_request, response, next) => {
    try {
        response.json(await Team.find().populate('members', 'username profile.displayName'));
    }
    catch (error) {
        next(error);
    }
...
router.get('/activities', async (request, response, next) => {
    try {
        ...
        response.json(await Activity.find(filter).populate('userId', 'username profile.displayName').sort({ createdAt: -1 }));
    }
    catch (error) {
        next(error);
    }
...
router.get('/leaderboard', async (_request, response, next) => {
    try {
        response.json(await Leaderboard.find()
            .sort({ rank: 1 })
